...
The editor shows two range sliders controlling the top and left position (in pixels). Values range from -200 to 200 and move one pixel at a time. Each slider displays its current value next to it. After tweaking the values, press **Save** to send them to `/variant/<id>/mockup-position`.
