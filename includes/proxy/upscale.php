...
$logContext = 'upscale_proxy';

// Lire le corps de la requête JSON
$inputJSON = file_get_contents('php://input');
customiizer_log($logContext, 'Reçu JSON: ' . $inputJSON);
...
if (!isset($input['hash']) || !isset($input['choice']) || !isset($input['webhook_url']) || !isset($input['webhook_type'])) {
        customiizer_log($logContext, 'Données manquantes dans la requête.');
        http_response_code(400);
...
customiizer_log($logContext, sprintf('Upscale demandé pour hash %s (choice %s)', $data['hash'], $data['choice']));
...
if ($response === false) {
        customiizer_log($logContext, 'Erreur cURL: ' . curl_error($ch));
...
if (json_last_error() !== JSON_ERROR_NONE) {
        customiizer_log($logContext, 'Erreur JSON dans la réponse: ' . json_last_error_msg());
        http_response_code(500);
        echo json_encode(['status' => 'error', 'message' => 'Réponse JSON mal formée de l\'API externe.']);
        exit;
}

customiizer_log($logContext, "Réponse de l'API externe: " . json_encode($responseData));
