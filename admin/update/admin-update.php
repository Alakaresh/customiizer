...
function customiizer_render_update_page() {
	...
	echo '    outputBox.innerText = "🔄 Démarrage de la mise à jour...\n";';
        ...
        echo '      };';
        echo '    })';
        echo '    .then(({ ok, status, statusText, text }) => {';
        echo '      if (!ok) {';
        echo '        const statusLabel = [status, statusText].filter(Boolean).join(" ") || "inconnue";';
        echo '        outputBox.innerText += `\n❌ Erreur HTTP ${statusLabel}`;';
        echo '        const trimmed = text.trim();';
        echo '        if (trimmed) {';
        echo '          const preview = trimmed.length > 800 ? `${trimmed.slice(0, 800)}…` : trimmed;';
        echo '          outputBox.innerText += `\n${preview}`;';
        echo '        }';
        echo '        return;';
        echo '      }';
        ...
        echo '          const payload = data.data || {};';
        echo '          const output = payload.output ? String(payload.output).trim() : "";';
        echo '          if (output) outputBox.innerText += "\n✅ Mise à jour terminée :\n" + output;';
        echo '          const release = payload.release ? String(payload.release).trim() : "";';
        echo '          if (release) outputBox.innerText += "\n📝 Notes de version :\n" + release;';
        echo '        } else {';
