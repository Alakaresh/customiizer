...
function customiizer_render_test_page() {
    ...
    echo '<script>\n';
    echo 'document.getElementById("start-loadtest").addEventListener("click", async function() {\n';
    echo '  var out = document.getElementById("loadtest-output");\n';
    echo '  out.textContent = "Test en cours...";\n';
    echo '  var res = await fetch(ajaxurl, {\n';
    echo '    method: "POST",\n';
    ...
    echo '  var txt = "";\n';
    echo '  data.data.results.forEach(function(r,i){\n';
    echo '    txt += (i+1) + ". " + (r.ok ? "OK" : "FAIL") + " " + r.time.toFixed(0) + " ms" + (r.error ? " - " + r.error : "") + "\\n";\n';
    echo '  });\n';
