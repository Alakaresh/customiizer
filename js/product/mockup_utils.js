...
(function (window) {
    // Global reference to the current product data
    window.productData = window.productData || null;
    ...
    function buildProductData(mockupData) {
        ...
        if (window.customizerCache) {
            ...
            window.customizerCache.designs[window.currentProductId] = window.productData;
    ...
    function updateMockupThumbnail(viewName, mockupUrl) {
        ...
        if (window.productData && viewName === getFirstMockup(selectedVariant)?.view_name) {
            window.productData.mockup_url = mockupUrl;
            if (!window.generatedProductId && !window.productCreationPromise) {
                window.productCreationPromise = createProduct(window.productData)
