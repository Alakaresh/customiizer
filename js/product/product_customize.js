...
jQuery(document).ready(function ($) {
        ...
        let initialDesignSnapshot = { canvasState: [], variantId: null };
        let initialVariantSnapshot = null;

        function cloneVariantData(variant) {
                ...
                try {
                        ...
                        console.warn('[Reset] Failed to clone variant data', error);
                        return { ...variant };
                }
        }
        ...
        function captureInitialSnapshotFromCanvas() {
                if (!window.CanvasManager || typeof CanvasManager.exportState !== 'function') {
                        initialDesignSnapshot.canvasState = [];
                } else {
                        try {
                                const exported = CanvasManager.exportState();
                                initialDesignSnapshot.canvasState = cloneCanvasStateLayers(exported);
                        } catch (error) {
                ...
                if (selectedVariant?.variant_id != null) {
                        initialDesignSnapshot.variantId = String(selectedVariant.variant_id);
                } else if (!initialDesignSnapshot.variantId) {
                        initialDesignSnapshot.variantId = null;
                }
        }
       ...
       function updateSidebarSelectedVariant() {
               ...
               const variant = sidebarVariants.find(v => {
                       const variantHex = typeof v.hexa === 'string' && v.hexa.trim()
                               ? v.hexa.trim().toLowerCase()
                               : null;
                       const matchesColor = (!selectedColor && !normalizedSelectedHex)
                               || (typeof v.color === 'string' && v.color === selectedColor)
        ...
        customizeButton.on('click', async function (event) {
                ...
                initialVariantSnapshot = cloneVariantData(selectedVariant);
                initialDesignSnapshot = {
                        canvasState: [],
                        variantId: initialVariantSnapshot?.variant_id != null
                                ? String(initialVariantSnapshot.variant_id)
                                : (selectedVariant?.variant_id != null ? String(selectedVariant.variant_id) : null)
                };
	...
	// 4) Ouvrir le sélecteur d’image
        ...
        resetDesignButton.on('click', async function () {
                ...
                try {
                        const targetVariantId = initialDesignSnapshot?.variantId || null;
                        const currentVariantId = selectedVariant?.variant_id != null
                                ? String(selectedVariant.variant_id)
                                : null;
                        const layersToRestore = cloneCanvasStateLayers(initialDesignSnapshot?.canvasState || []);
                        let variantSwitched = false;

                        if (targetVariantId && currentVariantId && targetVariantId !== currentVariantId && initialVariantSnapshot) {
                                const clonedVariant = cloneVariantData(initialVariantSnapshot);
                                if (clonedVariant) {
                                        selectedVariant = clonedVariant;
                                        await loadVariantInCustomizer(selectedVariant);
                                        await preloadSidebarVariants(selectedVariant);
                                        if (typeof window.applyVariantSelection === 'function') {
                                                try {
                                                        window.applyVariantSelection(selectedVariant);
                                                } catch (error) {
                                                        console.warn('[Reset] Failed to apply variant selection', error);
                                                }
                                        }
                                        const mainImage = $('#product-main-image');
                                        if (mainImage.length) {
                        ...
                        if (!variantSwitched && typeof CanvasManager.clearUserImages === 'function') {
                                CanvasManager.clearUserImages();
                        }

                        if (hasLayers && typeof CanvasManager.restoreState === 'function') {
                                try {
                                        ...
                                        if (typeof CanvasManager.clearUserImages === 'function') {
                                                CanvasManager.clearUserImages();
                                        }
                                }
                        }
