...
jQuery(function($) {
        ...
        function getGalleryWrapper() {
                return document.getElementById('generation-gallery');
        }

        function getPlaceholderGrid() {
                return document.getElementById('generation-placeholder-grid');
        }

        function getThumbnailsContainer() {
                return document.getElementById('generation-thumbnails');
        }

        function togglePreviewMode(isActive) {
                const previewWrapper = getPreviewWrapper();
                const galleryWrapper = getGalleryWrapper();
                const placeholderGrid = getPlaceholderGrid();
                ...
                if (galleryWrapper) {
                        galleryWrapper.classList.toggle('is-hidden', !active);
                        galleryWrapper.setAttribute('aria-hidden', active ? 'false' : 'true');
                }

                if (placeholderGrid) {
                        placeholderGrid.classList.toggle('is-hidden', active);
                        placeholderGrid.setAttribute('aria-hidden', active ? 'true' : 'false');
                }
        }
        ...
        function ensureThumbnailPlaceholders() {
                const thumbnailsContainer = getThumbnailsContainer();
                if (!thumbnailsContainer) {
                        return;
                }

                let thumbnails = Array.from(thumbnailsContainer.querySelectorAll('.generation-thumbnail'));

                if (thumbnails.length !== GENERATION_THUMBNAIL_LIMIT) {
                        thumbnailsContainer.innerHTML = '';
                        for (let i = 0; i < GENERATION_THUMBNAIL_LIMIT; i++) {
                                thumbnailsContainer.appendChild(createThumbnailPlaceholder(i));
                        }
                        return;
                }
        ...
        function renderGeneratedImages(images) {
                ...
                const thumbnailsContainer = getThumbnailsContainer();
                if (thumbnailsContainer) {
                        ensureThumbnailPlaceholders();

                        const thumbnails = Array.from(thumbnailsContainer.querySelectorAll('.generation-thumbnail'));
        ...
        function updateLivePreview(imageUrl) {
                ...
                const galleryWrapper = getGalleryWrapper();
                const shouldTogglePreview = galleryWrapper && !galleryWrapper.classList.contains('is-hidden');
